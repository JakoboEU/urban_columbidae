...
regional_pools = regional_pools[,c('jetz_scientific_name', 'city_name', 'city_id', 'present_in_city', 'origin')]
regional_pools
```
...
```{r}
regional_species = regional_pools[,c('jetz_scientific_name', 'city_id', 'city_name', 'origin')]
regional_species$record_type = 'Regional'

urban_species = regional_pools[regional_pools$present_in_city, c('jetz_scientific_name', 'city_id', 'city_name', 'origin')]
...
## Convex hull around all species
```{r fig.width=10, fig.height=10}
regional_hull <- species %>% filter(record_type == 'Regional') %>% slice(chull(trophic_trait, locomotory_trait))
urban_hull <- species %>% filter(record_type == 'Urban') %>% slice(chull(trophic_trait, locomotory_trait))
  
ggplot(data = species, aes(x = trophic_trait, y = locomotory_trait)) +
         geom_point(aes(color = record_type)) +
         geom_polygon(data = regional_hull, alpha = 0.2, color = 'green') +
         geom_polygon(data = urban_hull, alpha = 0.4, color = 'red') +
         theme_bw() + scale_color_manual(values=c("green", "red")) +
         xlab('Trophic Trait (Granivore -> Frugivore)') + ylab('Locomotory Trait (Migratory -> Non-Migratory)') + theme(legend.position="bottom")

ggsave('./figures/trait_space.jpg')
```

## City by city plots with convex hulls around trait space

```{r fig.width=10, fig.height=22}
plot_trait_space = function(realm_to_plot) {
  species_in_realm = species[species$realm == realm_to_plot,]
  species_in_realm$city_label = paste(species_in_realm$city_name, ' (', species_in_realm$city_id , ')', sep='')

  regional_hull <- species_in_realm %>% filter(record_type == 'Regional') %>% group_by(city_label) %>% slice(chull(trophic_trait, locomotory_trait))
  urban_hull <- species_in_realm %>% filter(record_type == 'Urban') %>% group_by(city_label) %>% slice(chull(trophic_trait, locomotory_trait))

  ggplot(data = species_in_realm, aes(x = trophic_trait, y = locomotory_trait)) +
         geom_point(aes(color = record_type, pch = origin)) +
         facet_wrap( ~ city_label, ncol=6) + 
         geom_polygon(data = regional_hull, alpha = 0.2, color = 'green') +
         geom_polygon(data = urban_hull, alpha = 0.4, color = 'red') +
         theme_bw() + 
         scale_color_manual(values=c("green", "red"), limits = c("Regional", "Urban")) +
         scale_shape_manual(values=c(16, 15), limits = c("Native", "Introduced")) +
         xlab('Trophic Trait (Granivore -> Frugivore)') + ylab('Locomotory Trait (Migratory -> Non-Migratory)') + theme(legend.position="bottom") +
         labs(title = realm_to_plot)
}

plot_trait_space('Palearctic')
ggsave('./figures/trait_space_palearctic.jpg')
```

```{r fig.width=10, fig.height=22}
plot_trait_space('Afrotropic')
ggsave('./figures/trait_space_afrotropic.jpg')
```


```{r fig.width=10, fig.height=22}
plot_trait_space('Australasia')
ggsave('./figures/trait_space_australasia.jpg')
```


```{r fig.width=10, fig.height=22}
plot_trait_space('Indomalayan')
ggsave('./figures/trait_space_indomalayan.jpg')
```


```{r fig.width=10, fig.height=22}
plot_trait_space('Nearctic')
ggsave('./figures/trait_space_nearctic.jpg')
```

```{r fig.width=10, fig.height=22}
plot_trait_space('Neotropic')
ggsave('./figures/trait_space_neotropic.jpg')
```