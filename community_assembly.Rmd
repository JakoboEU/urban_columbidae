...
```{r}
fd_fric(traits, species_in_cities)
```
...
```{r}
traits[species_names_in_514, c('mass')]
var(traits[species_names_in_514, c('mass')])
```
...
single_trait_variance = function(trait, communities) {
  ...
  for(row in rownames(communities)) {
    ...
    result = data.frame(city_id = row, variance = var(traits[species_names_in_community, c(trait)]))
