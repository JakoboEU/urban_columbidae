...
Normalised
```{r}
norm_mpd_analysis_plot_ro = geom_normalised_histogram(
  'MPD', 
  ggplot(urban_resident_only_species_community_result, aes(mpd_normalised)), 
  wilcox.test(urban_resident_only_species_community_result$mpd_normalised, mu = 0.5), 
  mean(urban_resident_only_species_community_result$mpd_normalised)
)
norm_mpd_analysis_plot_ro
```
...
```{r, fig.height=3, fig.width=3}
ggarrange(norm_mntd_analysis_plot_ro, norm_mpd_analysis_plot_ro, norm_fric_analysis_plot_ro, norm_fd_analysis_plot_ro, ncol = 2, nrow = 2)
```
...
## Calculate maximum possible diversity held by urban community if no Introduced species

Cities with introduced species
```{r}
city_ids_with_introduced_species = all_columbidae_communities$city_id[all_columbidae_communities$origin == 'Introduced' & all_columbidae_communities$present_in_city]
...
compare_all_metrics_actual_result_to_no_introduced = function(city_id, urban_richness, regional_species, actual_urban_species_community_result) {
  possible_community_values_with_no_introduced_species = get_community_metrics(
    create_null_model_communities(
      ...
      all_columbidae_communities$jetz_scientific_name[all_columbidae_communities$city_id == city_id & all_columbidae_communities$origin != 'Introduced']
    )
  )
...
for(no_introduced_loop_city_id in selected_urban_communties_with_introduced_species$city_id) {
  ...
  regional_species = all_columbidae_communities$jetz_scientific_name[all_columbidae_communities$city_id == no_introduced_loop_city_id & all_columbidae_communities$origin != 'Introduced']
...
### MPD
```{r}
compare_actual_to_no_introduced('MPD', 
  urban_no_introduced_species_community_result$mpd_max_possible_value_with_no_introduced_normalised,
  urban_no_introduced_species_community_result$mpd_actual_value_normalised,
  urban_no_introduced_species_community_result$city_id
)
```
### FD
